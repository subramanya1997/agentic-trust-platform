// Mock integration data with tools (will be replaced by API calls)
import type { Integration, IntegrationWithTools, Tool } from "@/lib/types/integrations";
      ...
      },
      {
        ...
        parameters: [
          { name: "lead_id", type: "string", description: "Salesforce Lead ID", required: true },
          {
            ...
            type: "boolean",
            description: "Whether to create an opportunity",
            required: false,
          },
          {
      ...
      },
      {
        ...
        parameters: [
          ...
          },
          {
            name: "types",
            type: "string",
            description: "Channel types (public, private)",
            required: false,
          },
        ],
      },
      ...
      },
      {
        ...
        parameters: [
          ...
          { name: "body", type: "string", description: "Comment text", required: true },
          {
            name: "public",
            type: "boolean",
            description: "Public or internal note",
            required: false,
          },
        ],
      },
        ...
        parameters: [
          {
            name: "type",
            type: "string",
            description: "Meeting type (scheduled, live)",
            required: false,
          },
        ],
      },
...
/**
 * Get an integration by its name (case-insensitive)
 */
export function getIntegrationByName(name: string): Integration | undefined {
  const normalizedName = name.toLowerCase();
  return INTEGRATIONS.find((integration) => integration.name.toLowerCase() === normalizedName);
}

/**
 * Get integration icon path based on theme
 * @param id - Integration ID or name
 * @param theme - Theme mode ('light' or 'dark'), defaults to 'light'
 * @returns Path to the integration icon
 */
export function getIntegrationIcon(id: string, theme: "light" | "dark" = "light"): string {
  const key = id.toLowerCase();

  // Try to find by ID first
  let integration = INTEGRATIONS.find((i) => i.id === key);

  // If not found, try by name
  if (!integration) {
    integration = INTEGRATIONS.find((i) => i.name.toLowerCase() === key);
  }

  if (!integration) {
    // Return default icon if integration not found
    return theme === "dark" ? "/integrations/dark/notion.svg" : "/integrations/light/notion.svg";
  }

  return integration.icons[theme];
}
 ...
 */
export function getIntegrationIcons(id: string): { light: string; dark: string } {
  const key = id.toLowerCase();
 ...
 */
export function getIntegrationsByType(type: "API" | "MCP"): Integration[] {
...
/**
 * Get all integrations that have tools defined
 */
export function getIntegrationsWithTools(): IntegrationWithTools[] {
  return INTEGRATIONS.filter(
    (integration): integration is IntegrationWithTools => integration.tools !== undefined
  );
}

/**
 * Get integration tools by integration ID
 */
export function getIntegrationTools(integrationId: string): Tool[] | undefined {
  const integration = INTEGRATIONS.find((i) => i.id === integrationId);
  return integration?.tools;
}

/**
 * Get integration with tools by ID (returns IntegrationWithTools if tools exist)
 */
export function getIntegrationWithToolsById(
  integrationId: string
): IntegrationWithTools | undefined {
  const integration = INTEGRATIONS.find((i) => i.id === integrationId);
  if (integration && integration.tools) {
    return integration as IntegrationWithTools;
  }
  return undefined;
}
